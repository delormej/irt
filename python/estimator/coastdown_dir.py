...
def fit_bike_power_by_drag_rr(mps, power):
	try:
		...
		print("Error fitting to drag/rr")
		return 0, 0
...
def get_files(dir):
	for root, dirs, files in os.walk(dir):
		for filename in files:
			if filename.endswith('.csv') and filename.startswith('calib_1.4.5'):
...
def compare_files(dir):
	...
	for file in get_files(dir):
		c = next(color_cycle)
		try:
			K, rr = graph_file(file)
			if (K > 0 and rr > 0):
				plt.plot(x*2.23694, drag_rr_func(x, K,rr), c)
				y_offset = y_offset+10
				plt.text(x_offset, y_offset, file, color=c, fontsize=8)
