...
@interface WFSensorCommonViewController ()
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if ( hardwareConnector.isCommunicationHWReady )
    {
        ...
        WFSensorConnection* sensor = nil;
        //
        for ( WFSensorConnection* conn in connections )
        {
            // check for network type match.
            if ( conn.networkType & applicableNetworks )
            {
                sensor = conn;
                break;
            }
...
- (void)firmwareUpdateAvailable:(NSNotification*)notification
{
    ...
    // implement in the main view controller
}
...
- (void)onSensorConnected:(WFSensorConnection*)connectionInfo
{
    ...
    if ( connectionInfo.networkType & applicableNetworks )
    {
	...
	switch (connState)
	{
		case WF_SENSOR_CONNECTION_STATUS_IDLE:
		{
			...
			if ( wildcardSwitch.on )
			{
				...
				params.sensorType = sensorType;
                params.networkType = applicableNetworks;
			}
			//
