...
int main()
{
	try
	{
		vector<int> servos = CServo::findServos(0x0424);
		//open first device
		CServo servo(0x0424, servos.at(0), 2/* DON'T ASK ME WHY */, 2/* I HAVE NO IDEA*/, CServo::BULK);
		...
		usb_handle = libusb_open_device_with_vid_pid(ctx, 0x0424, 0xec00);

		if (usb_handle == NULL)
		{
			throw "Could not open usb device.";
		}
...
// pi@raspberrypi ~/ src / c $ g++ program.c $(pkg - config --libs libusb - 1.0) - o program 
