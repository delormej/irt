...
static pm_info_t pm_info[POWER_METER_LIST_SIZE];
...
static bool pm_info_set_message(power_meter_info_t* p_pm_info, ANT_MESSAGE* p_mesg)
{
    ...
    switch (page_number)
    {
        case ANT_COMMON_PAGE_80:
            p_buffer = &(p_pm_info->ant_manufacturer_page);
            break;
            
        case ANT_COMMON_PAGE_81:
            p_buffer = &(p_pm_info->ant_product_page);
            break;

        default:
            return false;
    }
    memcpy(p_buffer, p_mesg->aucMesgData, TX_BUFFER_SIZE);
    return true;
}
...
static uint16_t device_id_get(ANT_MESSAGE* p_mesg)
{
    ...
    // TBD -- need to figure out where this is
    return (uint16_t)(p_mesg->aucExtData);
}
...
static bool read_page(ant_evt_t * p_ant_evt)
{
    ...
    if (device_id > 0)
    {
        ...
        new_pm_info |= size > original_size;
    }
    return new_pm_info;
}
