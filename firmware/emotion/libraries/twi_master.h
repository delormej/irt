...
/* TWI specifies which TWI device, 0 or 1 */
/*lint -e717 -save "Suppress do {} while (0) for these macros" */
/*lint ++flb "Enter library region" */
#define TWI_SCL_HIGH(TWI)   do { NRF_GPIO->OUTSET = (1UL << TWI_MASTER_CONFIG_CLOCK_PIN_NUMBER[TWI]); } while(0)   /*!< Pulls SCL line high */
#define TWI_SCL_LOW(TWI)    do { NRF_GPIO->OUTCLR = (1UL << TWI_MASTER_CONFIG_CLOCK_PIN_NUMBER[TWI]); } while(0)   /*!< Pulls SCL line low  */
#define TWI_SDA_HIGH(TWI)   do { NRF_GPIO->OUTSET = (1UL << TWI_MASTER_CONFIG_DATA_PIN_NUMBER[TWI]);  } while(0)   /*!< Pulls SDA line high */
#define TWI_SDA_LOW(TWI)    do { NRF_GPIO->OUTCLR = (1UL << TWI_MASTER_CONFIG_DATA_PIN_NUMBER[TWI]);  } while(0)   /*!< Pulls SDA line low  */
#define TWI_SDA_INPUT(TWI)  do { NRF_GPIO->DIRCLR = (1UL << TWI_MASTER_CONFIG_DATA_PIN_NUMBER[TWI]);  } while(0)   /*!< Configures SDA pin as input  */
#define TWI_SDA_OUTPUT(TWI) do { NRF_GPIO->DIRSET = (1UL << TWI_MASTER_CONFIG_DATA_PIN_NUMBER[TWI]);  } while(0)   /*!< Configures SDA pin as output */
#define TWI_SCL_OUTPUT(TWI) do { NRF_GPIO->DIRSET = (1UL << TWI_MASTER_CONFIG_CLOCK_PIN_NUMBER[TWI]); } while(0)   /*!< Configures SCL pin as output */
/*lint -restore */

#define TWI_SDA_READ(TWI) ((NRF_GPIO->IN >> TWI_MASTER_CONFIG_DATA_PIN_NUMBER[TWI]) & 0x1UL)                     /*!< Reads current state of SDA */
#define TWI_SCL_READ(TWI) ((NRF_GPIO->IN >> TWI_MASTER_CONFIG_CLOCK_PIN_NUMBER[TWI]) & 0x1UL)                    /*!< Reads current state of SCL */
 ...
 */
bool twi_master_transfer(uint8_t twi, uint8_t address, uint8_t *data, uint8_t data_length, bool issue_stop_condition);
